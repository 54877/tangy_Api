...
const userEmail = async (email: string, ErrorMessage: string) => {
...
export const sendEmail = async (email: string) => {
  const user = await userEmail(email, "無效請求");
...
export const newPasswordLogic = async (
  ...
  if (!record) {
    throw new AppError("驗證失敗", 400);
  }

  if (record.expires_at <= new Date()) {
    await expiredCodeDb(email);
    throw new AppError("驗證碼已過期", 400);
  }

  if (record.count >= 5) {
    await expiredCodeDb(email);
    throw new AppError("嘗試次數過多，請重新取得驗證碼", 429);
  }

  if (record.code !== code) {
    await codeCountDb(email);
    throw new AppError("驗證碼錯誤", 400);
  }

  //驗證密碼是否與舊密碼相同
  const user = await userEmail(email, "驗證失敗");
  const psd = await bcrypt.compare(newPassword, user.password);
  if (psd) {
    throw new AppError("新舊密碼不可相同", 400);
  }
